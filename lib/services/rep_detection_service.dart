...
class RepDetectionService {
  ...
  RepDetectionService({this.threshold = 1.0, this.onRepCompleted});
  ...
  void _endRep(DateTime timestamp, String side) {
    ...
    if (_repStartTime != null && _currentRepWeights.isNotEmpty) {
      final avgWeight =
          _currentRepWeights.reduce((a, b) => a + b) /
          _currentRepWeights.length;
  ...
  /// Get current rep average (if in rep)
  double get currentRepAverage {
    if (!_inRep || _currentRepWeights.isEmpty) return 0.0;
    return _currentRepWeights.reduce((a, b) => a + b) /
        _currentRepWeights.length;
  }

  /// Get current rep median (if in rep)
  double get currentRepMedian {
    if (!_inRep || _currentRepWeights.isEmpty) return 0.0;
    return Rep.calculateMedian(_currentRepWeights);
  }

  /// Get current rep duration (if in rep)
  Duration get currentRepDuration {
    if (!_inRep || _repStartTime == null) return Duration.zero;
    return DateTime.now().difference(_repStartTime!);
  }
